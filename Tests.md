...
3. Тест на внутренние кавычки (✓✓✓)
   ...
   - Результат: токен распознается корректно как TOKEN_WORD, executor пытается выполнить его как команду (стандартное поведение Unix shell).
...
8. Неизвестные или некорректные символы (??✓)
   ...
   - **Проблема**: символ `&` интерпретируется как фоновый оператор, создается job `echo $%^`.
9. Пустые кавычки (✓✓✓)
   - Ввод: `echo "" ''`
...
4. Тест на произвольный порядок и количество редиректов (✓??)
   ...
   - **Проблема**: редиректы распознаны также как в bash но флаг требуется после самой команды.
...
4. Тест на отсутствие команды перед и между операторами (✓✓✓)
   - Ввод: `| grep "txt" || wc -l`
   - Ожидаемый результат: ошибка синтаксиса
   - Результат: parser корректно выдает ошибку `Parser error: unexpected '|' at beginning`
...
8. Тест на нотификацию пользователя о завершении/остановке фоновой задачи (✓✓✓)
   - Ввод: `sleep 5 &` затем ожидание завершения
   - Ожидаемый результат: после завершения задачи шелл выводит сообщение о завершении задачи с её номером и PID.

## Тест встроенных команд

1. Тест на команду `cd` (✓✓✓)
   - Ввод: `cd /tmp`
   - Ожидаемый результат: текущая рабочая директория изменяется на `/tmp`. При ошибке (например, несуществующая директория) выводится сообщение об ошибке.
   - Продолжение: `cd` без аргументов должно менять директорию на домашнюю директорию пользователя.
   - Продолжение: `cd -` должно менять директорию на предыдущую рабочую директорию.
   - Продолжение: `cd` должен менять переменную окружения `PWD` на текущую рабочую директорию после успешного изменения директории.
2. Тест на команду `set` и `VAR=VALUE` (✓✓✓)
   - Ввод: `MYVAR=myvalue` или `set MYVAR=myvalue`
   - Ожидаемый результат: переменная `MYVAR` создается или обновляется в окружении шелла с значением `myvalue` (не является переменной окружения)
3. Тест на команду `unset` (✓✓✓)
   - Ввод: `unset MYVAR`
   - Ожидаемый результат: переменная `MYVAR` удаляется из окружения шелла. Если переменная не существует, ошибок не возникает.
4. Тест на команду `export` (если реализована) (✓✓✓)
   - Ввод: `export MYVAR`
   - Ожидаемый результат: переменная `MYVAR` экспортируется в окружение дочерних процессов. Если переменная не существует, выводится сообщение об ошибке.
5. Тест на команду `exit` (✓✓✓)
   - Ввод: `exit 0`
   - Ожидаемый результат: шелл завершает работу с кодом возврата 0.
   - Продолжение: `exit` без аргументов должно завершать работу шелла с кодом возврата последней выполненной команды.
   - Продолжение: `exit 42` должно завершать работу шелла с кодом возврата 42.
   - Ожидаемая реализация: выставления флага завершения основного цикла и корректное освобождение ресурсов перед выходом.
6. Тест на команду `pwd` (✓✓✓)
   - Ввод: `pwd`
   - Ожидаемый результат: выводится текущая рабочая директория.
7. Тест на команду `echo` (✓✓✓)
   - Ввод: `echo Hello World`
   - Ожидаемый результат: выводится строка `Hello World`.
   - Продолжение: `echo $MYVAR` должно выводить значение переменной `MYVAR`, если она установлена.
8. Тест на команду `env` (если реализована) (✓✓✓)
   - Ввод: `env`
   - Ожидаемый результат: выводится список всех переменных окружения и их значений.
9. Тест на команду `history` (✓✓✓) (если реализована)
   - Ввод: `history`
   - Ожидаемый результат: выводится список ранее введённых команд с их номерами.
10. Тест на команду `help` (✓✓✓)
	- Ввод: `help`
   - Ожидаемый результат: выводится список всех встроенных команд с кратким описанием их функциональности.
11. Тест на команду `jobs` (✓✓✓)
	- Ввод: `jobs`
   - Ожидаемый результат: выводится список всех текущих задач с их статусами (выполняется, остановлена).
12. Тест на команду `fg` (✓✓✓)
	- Ввод: `fg %1`
   - Ожидаемый результат: задача с номером 1 переводится на передний план и продолжает выполнение. Все остановленные задачи возобновляются.
   - Продолжение: `fg` без аргументов должно переводить на передний план последнюю остановленную задачу.
13. Тест на команду `bg` (✓✓✓)
	- Ввод: `bg %1`
	   - Ожидаемый результат: задача с номером 1 возобновляется в фоне.
	   - Продолжение: `bg` без аргументов должно возобновлять последнюю остановленную задачу в фоне.
14. Тест на команду `kill` (✓✓✓)
	- Ввод: `kill %1` или `kill pid/pgid`
   - Ожидаемый результат: отправляется сигнал TERM (по умолчанию) указанной задаче или процессу. При ошибке (например, несуществующий PID) выводится сообщение об ошибке.
   - Продолжение: `kill %1 sig` или `kill pid/pgid sig` должно отправлять указанный сигнал.

## Тесты на управление памятью и ресурсами
- Тест на утечки памяти при вводе и выполнении команд (✓✓✓)
   - Ввод: серия команд с различными аргументами, редиректами и конвейерами
   - Ожидаемый результат: после выполнения каждой команды или серии команд не должно быть утечек памяти (проверяется с помощью инструментов типа Valgrind).
- Тест на корректное освобождение памяти при ошибках (✓✓✓)
   - Ввод: команды с синтаксическими ошибками или некорректными аргументами
   - Ожидаемый результат: при возникновении ошибок память должна быть корректно освобождена
- Тест на файльные дескрипторы (✓✓✓)
   - Ввод: команды с редиректами и конвейерами
   - Ожидаемый результат: все открытые файловые дескрипторы должны быть корректно закрыты после выполнения команд или при возникновении ошибок.
- Тест на завершение shell (✓✓✓)
   - Ввод: команда `exit` или `CTRL+D`
   - Ожидаемый результат: при завершении работы шелла все ресурсы (память, файловые дескрипторы, запущенные процессы) должны быть корректно освобождены и закрыты.